...
if __name__ == '__main__':
    # google map api
    gmaps = googlemaps.Client(key='AIzaSyBIVQBUQOGsxBrxTDe7blqiplVDdb0mvDg')
    #cloud couchdb
    couch = couchdb.Server('http://115.146.89.67:5984')
    cdb = couch['melbourne_ccc']
    view = cdb.view("crime/crime_tweet_geocode")
    crime_cdb = couch['crime_postcode_ccc']

    print view
    for row in view:
        id = row["value"]["_id"]
        tweet = row["value"]
        if not id in crime_cdb:
            # print id
