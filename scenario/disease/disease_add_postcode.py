...
if __name__ == '__main__':
    # google map api
    gmaps = googlemaps.Client(key='AIzaSyCkGOniAh0e3h5yJ1gcVP6ep1DqbIrUdzM')
    #cloud couchdb
    couch = couchdb.Server('http://115.146.89.67:5984')
    cdb = couch['melbourne_ccc']
    view = cdb.view("disease/disease_tweet_geocode")
    disease_cdb = couch['disease_postcode_ccc']

    print view
    for row in view:
        id = row["key"]
        tweet = row["value"]
        if not id in disease_cdb:
            # print id
